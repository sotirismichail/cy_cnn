...
def generate(out_channels: int = 3,
    ...
    for k in range(out_channels):
        kernel = np.around(np.random.normal(0, 1, size=(kernel_size, kernel_size, kernel_size)), decimals=4)
...
def weights(in_channels: int = 3,
    ...
    """
    Description:
        Calculating the learnable weights of the module of shape (out_channels, in_channels/groups, kernel_size,
